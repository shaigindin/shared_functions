...
def tagRepo(paramMAp, repo_name, package_type){
	...
	withCredentials([gitUsernamePassword(credentialsId: '30bac85c-db0f-430c-9cd0-6bd25f2eb01a', gitToolName: 'git-tool')]) {
        bat(script: "python ${paramMAp.WORKSPACE}/pycommon/misc/version_util.py --command 3 --repo_path ${paramMAp.WORKSPACE}/${repo_name} --type ${package_type} --version_build_number ${currentBuild.number}", label : "tag ${repo_name} repository")
    }
}
